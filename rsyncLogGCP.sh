...
tail -f -n0 ${rsyncLog}| while read line; do
        ...
        if [[ ${fileName} == 'receiving file list'  ]];then
                ...
                ${goApiDir}/gcpStorageAPI -b ${gs} -f ${taskListDir}/${dateUpload}-$taskID-cache -m upload -p ${srcDir}/ -t 20 >> ${taskLogDir}/${dateUpload}-$taskID.log
                echo '开始上传不需要缓存的文件' >> ${taskLogDir}/${dateUpload}-$taskID
                ${goApiDir}/gcpStorageAPI -b ${gs} -f ${taskListDir}/${dateUpload}-$taskID-noCache -m upload -c false -p ${srcDir}/ -t 20 >> ${taskLogDir}/${dateUpload}-$taskID.log
        else
                addTask
        fi
done
